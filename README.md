...
> 2. In the new repository (i.e., the fork), check that all your team members have access by going to the `settings` tab and then clicking on `Collaborators and Teams` in the left side bar. You should see your team there with write access.
> 3. Once you reserve a demo slot, you will also need to give the TA/reader that will be taking your demo admin access to your repository for grading. From  `settings` choose `Collaborators and Teams` from the left side bar. Then, click on the green button `Add people`, and add the TA/reader from the list below using their GitHub username. Make sure you give them `admin` permission to the repository. Please make sure you do this step **_before_** the demo time starts and not during the actual demo.
>    * Hallie Pham (halliehp)
>    * Jordan Chan (jchan420)
>    * Joshua Candelaria (jecndlria)
